...
public class DateUtils {
	...
	public static final String DATE_FORMAT_YMD_OTHER = "yyyy/MM/dd";
	...
	/**
	 * 字符串转换为制定格式日期 (注意：当你输入的日期是2014-12-21 12:12，format对应的应为yyyy-MM-dd HH:mm
	 * 否则异常抛出)
	 * 
	 * @param date
	 * @param format
	...
	public static String getDateToMinute(String fullDateStr) {
		return fullDateStr == null ? null
				: (fullDateStr.length() >= DATE_LENGTH ? fullDateStr.substring(
						0, DATE_LENGTH) : fullDateStr);
	}
	 ...
	 * 获取指定日期星期几
	 * 
	 * @param dateTime yyyy-MM-dd格式的日期字符串
	 * @throws ParseException
	 */
	...
	/**
	 * 获取指定日期的最大日期 时间2014-12-21 23:59:59
	 *
	 * @return
	 */
	...
	/**
	 * 获取指定日期的最小日期时间 2014-12-21 00:00:00
	 *
	 * @return
	 */
	 ...
	 * @return
	 */
	public static Integer DateOfWeek(Date date) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
	 ...
	 * 当前日期为一个月的第几天
	 * */
	public static Integer DateOfMonth(Date date) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
	...
	private static final String DEFAULT_FORMAT_HTML = "yyyy-MM-dd";
	...
	private static Date getNextWeekMonday(Date date) {
		Calendar cal = Calendar.getInstance();
	...
	private static Date getNextMonthFirstDate(Date date){
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
